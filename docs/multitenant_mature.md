...
| B2 | ✅ | First-user-becomes-admin is a prod risk | Onboarding is **invite-only** — open self-registration is removed. The platform superadmin sends the single workspace-owner bootstrap invite (`POST /admin/tenants/{id}/invites`); that invite redeems into the workspace Superadmin — a platform-managed role (all actions) that cannot be renamed, edited, deleted, or assigned/demoted by the tenant. The Superadmin invites Admins (assistants) and members via `/tenant/invites`. |
